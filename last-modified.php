<?php
	...
	// After beating my head against the wall, I found this fully hashed out function that did exactly what I was trying to do.
	// The function checks the cURL to find the mp3 file's last modified Timestamp, and if that comes back empty, it checks the PHP header.

	// Credit: Andrew Baillie
  // URI: https://github.com/andrewbaillie/Last-Modified-Date-of-URL

	// Samples

	// Return Timestamp
	// $result_1 = remote_file_last_modified( 'https://media.publicbroadcasting.net/wjct/newscast/newscast.mp3' );

	// Return Human Readable Date (Default)
	// $result_2 = remote_file_last_modified( 'https://media.publicbroadcasting.net/wjct/newscast/newscast.mp3' , false );

	// Return Human Readable Date (Modified)
	// $result_3 = remote_file_last_modified( 'https://media.publicbroadcasting.net/wjct/newscast/newscast.mp3' , false , 'jS F Y H:ia' );
	...
	function remote_file_last_modified( $url , $timestamp = true, $date_format = 'd-m-Y H:i:s' ) {
    	...
    	if ( $curl_result !== FALSE ) {
    		...
    		$headers = explode( "\n" , $curl_result );
				...
				// find the index key of the last modified timestamp in the array
				$needle = 'Last-Modified: ';
				$myindex = array_find($needle, $headers);

				// Extract the last modified date from the headers

				// $last_modified = explode( 'Last-Modified: ' , $headers[5] );
				$last_modified = explode( 'Last-Modified: ' , $headers[$myindex] );
